...
Note, if this is the first time, it will have to download and install a number of packages. Wait a few minutes for it to complete. Then you can close the red text output by clicking the X in the right corner above the text. 

```{r}
if (!require("tidyverse")) install.packages("tidyverse") #load tidyverse, which includes dplyr and ggplot2. Note, if already installed, the shorter code will work too: library(tidyverse)
```
