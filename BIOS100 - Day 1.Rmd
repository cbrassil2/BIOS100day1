...
In many cases, asking and answering questions in biology means collecting data on cells, tissues, organisms, populations, or ecosystems. In your laboratory courses, you will have the time and resources to begin learning those methods. However, there are many questions in biology in which we make progress by analyzing existing data, and sometimes large data sets. There tremendous amounts of genetic data that we generated across individual research projects, which is now stored in central databases. The National Ecological Observatory Network (NEON) has been collected data on ecosystem processes at 81 sites across the United States since 2019. Research questions that utilize existing data allows us, in short introductory course, to jump right into the heart of asking and answering questions about life. At the same time, you will add some skills from data science to toolbox, something which you may draw in your future courses or career.
...
What you are viewing is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

In R Markdown, the R code itself is contained in a "chunk" that look like this. To execute this chunk and display an output, either click on the green arrow in the far right, or place your cursor anywhere in the gray area and type the following keys together: Ctrl+Enter in Windows or Cmd+Return in Mac.  
...
In R, you create a vector of numbers, which is essentially a form of a list, by using the c() function, which stands for combine. 

```{r}
c(4,5,2,11)
```

A programming language is much more powerful than a calculator because we can store numbers in variable, and then reference that variable. There are different ways you can do this in R, with either a single equal sign or an arrow (typed using the minus sign and > or <). I prefer the arrow because it more explicitly communicate that you are storing a number in a variable. In addition, arrows have the advantage that can be used in either direction.
```{r}
x = 6
y <- 5
4 -> fred
...
c(4,5,2,11)
```
In this case, the output does not add any decimal point. In your project you may need to be aware of whether of the type of data. It generally will look at everything in a set of data and make it all the same type. For example, in the following vector I have add characters as a quoted string.

```{r}
c("one","three",4, 5.6)
```

In the output, you see that everything now has quotes becuase it interpretted everything as characters.
...
One of the specific packages will use is called dplyr, which gets its name comes from the data pliers. In other words, tools you can use to grab and manipulate data, shortened to dplyr.
...
```
In base R we could simplify this a bit by skipping the step of storing the vector in x, but you have nest a series of steps and read it from the inside out.
...
The pipe takes the output from each line and places it in the first position of the function listed in the next line. This has two advantages. 1) You don't have to create a bunch or arbitrarily named variables. 2) You can read the code from bottom to top and think about each step modifying the output of the previous step. In other words, like data flowing down a water pipe, being manipulated at each step.
...
In one more twist of complications, the above doesn't actually show us any data. It just lays out the axes. To plot the data points we have to use one more function, in this case geom_point() to plot a scatterplot. The name is a bit esoteric, but the "geom" part refers to the fact that we are making geometric object and the "point" part refers to the fact that we want to plot points.

And in one more annoying twist of coding, when we are modifying output from ggplot() we have to use a "+" sign instead of "%>%". There is no good reason for this, it is artificant of how these functions developed over time.
...
Modify the code to capitalize the name of the y-axis as well. Hint, the function is called ylab()
...
Even I don't walk around with all of these details in my head. To make the above graph, I had to Google "R bar plot". I ended up using this website <http://www.sthda.com/english/wiki/ggplot2-barplots-quick-start-guide-r-software-and-data-visualization>, which showed my how to includ"e the stat="identity" part in geom_bar. Looking up example code in Google is very useful. It is probably the number way I look for coding information. A simple search such as "tidyverse summarise" quickly gets you to useful sites.

And the online references linked through the tidyverse website really great
- dplyr <https://dplyr.tidyverse.org/>
- ggplot2 <https://ggplot2.tidyverse.org/>

Even more than looking up code via Google, I do a lot of copy/paste modify existing code. You can do the same. Copy the code from this file and future tutorials. Then modify the code to make it do what you want.
...
Note, we've introduced a new function above, and I suspect you can figure out what it does. In words, describe what does the above code does?
...
This uses geom_jitter, which is a version of geom_point, i.e. the scatterplot, which adds or subtracts a random value to shift potentially overlappign points so that you can see them all. Hint, the bug in this code is not within "geom_jitter(shape = 1)". That part is correctly used in this code.

```{r}
iris %>%
  ggplot(aes(x=Sepal.Length, y=Sepal.Width)) %>%
  geom_jitter(shape = 1) %>% #scatterplot which adds a small random value to each point
  theme_classic()
```
